...
- **A badge that says it is working** — the extension replaces text in place, so once it works the page looks like a page Google rendered correctly. The toolbar icon therefore carries the number of names applied to the page you are looking at, with a tooltip saying so, and nothing at all on a page with nothing to rename. Per tab, and it clears when you leave Google Analytics or move to a property that has no name yet
- **Toolbar popup** — left-click the extension icon to view and edit mappings from any tab without leaving your current page, and run either naming action from there: **Fill missing names** runs in the popup, and **Visit and name N more** hands over to the settings page, because Chrome destroys a popup as soon as it loses focus
