...
| `propertyAccounts` | local | `{ [slug]: accountId }` | content script | Account each property belongs to, from GA4's inline tree and from the URL. Groups the settings table, and **populates** it: a property listed here with no name in either map still gets an empty row, which is the only place the user can name it by hand. Never used for replacement. |
