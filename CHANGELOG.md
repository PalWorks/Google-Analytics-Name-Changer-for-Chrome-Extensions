...
- **An account holding two or more extensions says why it is unnamed** — such an account is deliberately never auto-named, because naming it after one of several would be arbitrary. That rule was only stated in the explainer card, so the blank field read as a bug. The row itself now says "Name this account yourself (holds N extensions)"
